...
function SearchBar() {
    ...
    const getWeather = async (searchQuery) => {
        ...
        if (searchQuery !== "") {
            try {
                await axios.get(`https://api.weatherapi.com/v1/search.json?q=${searchQuery}&key=${import.meta.env.VITE_API_KEY}`)
                    .then(result => {
    ...
    return (
        <div className=' pt-7 mx-0 sm:pt-10 flex items-center justify-center '>
            <div>
                <div className='flex  bg-white w-[900px] sm:w-[300px] md:w-[375px] lg:w-[700px] items-center justify-between  rounded-xl shadow-2xl shadow-black' >
                    <input type="text" value={searchQuery} onChange={handleInputChange} placeholder="Search for city or state"
                        className="  px-6 py-3 w-[850px] sm:px-3 sm:py-1 sm:w-[275px] md:w-[350px] lg:w-[700px] rounded-xl sm:text-sm md:text-base lg:text-normal text-lg focus:outline-none placeholder:text-black" />
                        ...
                        weatherData.map((weatherdata, index) => {
                            return (
                                <div key={index} className=' z-50'>
                                    <li className="py-3 sm:py-1 cursor-pointer list-none  " >
